...
int countPairs(map<int, int> sockMap)
{
	int pairCount = 0;
	//cout << "Pairs:" << endl;
	for(map<int, int>::iterator it = sockMap.begin(); it != sockMap.end(); ++it)
	{
...
int sockMerchant(int n, vector<int> ar) {
	...
	{
		if(sockMap.count(ar[i]) > 0)
			sockMap[ar[i]] = sockMap[ar[i]] + 1;
		else
			sockMap[ar[i]] = 1;
	}

	return countPairs(sockMap);
}
